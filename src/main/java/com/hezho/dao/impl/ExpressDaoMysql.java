...
public class ExpressDaoMysql implements BaseExpressDao {
    ...
    public static final String SQL_CONSOLE = "SELECT \n" +
            ...
            "(SELECT COUNT(TO_DAYS(INTIME)=TO_DAYS(NOW()) OR NULL)) AS data1_day,\n" +
            "(SELECT COUNT(STATUS=0 OR NULL)) AS data2_size,\n" +
            "(SELECT COUNT(TO_DAYS(INTIME)=TO_DAYS(NOW()) AND STATUS=0 OR NULL)) AS data2_day \n" +
            "FROM eEXPRESS;";

    //用于查询数据库中的所有快递信息
    public static final String SQL_FIND_ALL = "SELECT * FROM eEXPRESS";
    //用于分页查询数据库中的快递信息
    public static final String SQL_FIND_LIMIT = "SELECT * FROM eEXPRESS LIMIT ?,?";
    //通过取件码查询快递信息
    public static final String SQL_FIND_BY_CODE = "SELECT * FROM eEXPRESS WHERE CODE=?";
    //通过快递单号查询快递信息
    public static final String SQL_FIND_BY_NUMBER = "SELECT * FROM eEXPRESS WHERE NUMBER=?";
    //通过录入人手机号查询快递信息
    public static final String SQL_FIND_BY_SYSPHONE = "SELECT * FROM eEXPRESS WHERE SYSPHONE=?";
    //通过用户手机号查询用户所有快递
    public static final String SQL_FIND_BY_USERPHONE = "SELECT * FROM eEXPRESS WHERE USERPHONE=?";
    //录入快递
    ...
    public static final String SQL_FIND_BY_USERPHONE_AND_STATUS = "SELECT * FROM eEXPRESS WHERE USERPHONE=? AND STATUS=?";
    /**
     * 用于查询数据库中的全部快递（总数+新增），待取件快递（总数+新增）
     *
